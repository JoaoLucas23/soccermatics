# -*- coding: utf-8 -*-
"""
Created on Fri Nov 18 00:06:57 2022
...
file_path = "H:/Documentos/SaLab/Soccermatics/playerstats.csv"
#num_obs=20
...
fig.suptitle("Linear regression: Quadratic Model", fontsize=16)
plt.show()


##############################################################################
# A model including cubic terms
# ----------------------------
#We now fit the cubic model
# .. math::
#
...
minutes_model = minutes_model.assign(age_cubic=np.power(laliga_df['Age'],3))

model_fit=smf.ols(formula='minutes  ~ age + age_squared + age_cubic ', data=minutes_model).fit()
print(model_fit.summary())        
b=model_fit.params
...
#for i,a in enumerate(minutes_model['age']):
    #ax.plot([a,a],[minutes_model['minutes'][i], b[0] + b[1]*a + b[2]*a*a + b[3]*a*a*a], color='red')
fig.suptitle("Linear regression: Cubic Model", fontsize=16)
plt.show()